...
p3RayTracer = PurkinjeModel("./data/p3_infinity.zos", rotationSurfaceIdx)
if not p3RayTracer.isFileOpen:
    raise FileNotFoundError("File not found.")
p3RayTracer.setEyeRotationX(0)

MAX_EYE_ROTATION = 15.0
MAX_SOURCE_ANGLE = 15.0
SURFACES = np.array([1, 4, 5, 6, 7, 8, 12, 13, 14, 15], dtype=int)
...
for aIdx, sourceAngle in enumerate(SOURCE_ANGLE):
    for rIdx, rot in enumerate(EYE_ROTATION):
        p3RayTracer.setEyeRotationX(rot)
        rays[aIdx, rIdx, ...] = \
            p3RayTracer.trace(sourceAngle, surfaces=SURFACES)
        
np.savez_compressed('./data/p3INF_S15_R15.npz', rays=rays)
...
with open("./data/p3INF_S15_R15.pk", 'wb') as handle:
    pickle.dump(
        data,
