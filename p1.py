...
p1RayTracer = PurkinjeModel("./data/p1.zos", rotationSurfaceIdx)
if not p1RayTracer.isFileOpen:
    raise FileNotFoundError("File not found.")
p1RayTracer.setEyeRotationX(0)

MAX_EYE_ROTATION = 15.0
MAX_SOURCE_ANGLE = 15.0
SURFACES = np.array([1, 4, 8, 9, 10, 11], dtype=int)
...
for aIdx, sourceAngle in enumerate(SOURCE_ANGLE):
    for rIdx, rot in enumerate(EYE_ROTATION):
        p1RayTracer.setEyeRotationX(rot)
        rays[aIdx, rIdx, ...] = \
            p1RayTracer.trace(sourceAngle, surfaces=SURFACES)
        
np.savez_compressed('./data/p1_S15_R15.npz', rays=rays)
...
with open("./data/p1_S15_R15.pk", 'wb') as handle:
    pickle.dump(
        data,
